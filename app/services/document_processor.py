...
class DocumentProcessor:
            ...
            with pdfplumber.open(BytesIO(file_content)) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages):
                    try:
                        ...
                        logger.warning(f"pdfplumber: Error on page {page_num + 1}: {str(page_error)}")
                        continue
        ...
        try:
            ...
            for page_num, page in enumerate(pdf_reader.pages):
                try:
                    ...
                    logger.warning(f"PyPDF2: Error on page {page_num + 1}: {str(page_error)}")
                    continue
            ...
            if text.strip():
                ...
                raise ValueError("No readable text found in PDF. The PDF might be image-based, encrypted, or corrupted.")
        
        except Exception as e:
            logger.error(f"PyPDF2 also failed: {str(e)}")
            raise ValueError(f"PDF text extraction failed with both methods. The PDF might be image-based, encrypted, password-protected, or corrupted. Error: {str(e)}")
    ...
    def chunk_text(self, text: str) -> List[str]:
        ...
        words = text.split()
        logger.info(f"Chunking text with {len(words)} words (chunk_size={self.chunk_size}, overlap={self.chunk_overlap})")
    ...
    def process_file(self, file_content: bytes, file_name: str) -> List[str]:
        ...
        if not text or len(text.strip()) < 10:
            raise ValueError(f"Insufficient text content extracted from {file_name}. Got {len(text)} characters.")
